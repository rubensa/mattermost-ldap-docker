...
psql -d $oauth_db_name -c "$create_table_oauth_client"
psql -d $oauth_db_name -c "$create_table_oauth_access_tokens"
psql -d $oauth_db_name -c "$create_table_oauth_authorization_codes"
psql -d $oauth_db_name -c "$create_table_oauth_refresh_tokens"
psql -d $oauth_db_name -c "$create_table_users"
psql -d $oauth_db_name -c "$create_table_oauth_scopes"

#Insert new client in the database
info "Insert new client in the database"
psql -d $oauth_db_name -c "$create_client"

#Verification
psql -d $oauth_db_name -c "SELECT * from oauth_clients WHERE client_id='$client_id';" | grep '(1'
